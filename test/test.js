...
var image = new Jimp(video_size.width, video_size.height, 0xFF0000FF, function (dx, buf) {
  return function (err, mg) {
...
for (var i = 0; i < 10; i++) {
  ...
  var frameBuf = fencoder.getRGBABufferOfFrame();
  image.bitmap.data = frameBuf;
  image.write("./out_" + i + ".png", function (st) {
    return function () {
      console.log("FRAME TIME: ", (new Date()) - st);
    };
  }(nw));
